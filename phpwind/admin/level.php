...
if (empty($action)) {
	...
	if (empty($step)) {
		...
		foreach ($uploadtype as $key => $value) {
			...
			<td><input class=\"input\" size=\"10\" name=\"filetype[]\" value=\"$key\"></td>
			<td><input class=\"input\" size=\"10\" name=\"maxsize[]\" value=\"$value\"> <a style=\"cursor:pointer;color:#FA891B\" onclick=\"removecols(this);\">[删除]</a></td>
			</tr>";
		}
		...
		*/
		...
		//display push time input
		$ptdisplay = $pushadmin_Y && $pushadmin_Y == 'checked' ? '' : 'style=display:none';
		...
		InitGP(array('othergroup','group','othergid','filetype'),'P');
	...
	if (empty($_POST['step'])) {
		...
		$sql = '';
		if ($keytype == 'special') {
			$sql .= " AND gptype='special'";
		} elseif ($keytype <> 'basic') {
			$sql .= " AND gptype>2";
		}
		$query = $db->query("SELECT gid,gptype FROM pw_usergroups WHERE 1 $sql");
		while ($rt = $db->fetch_array($query)) {
			$gdb[] = $rt['gid'];
		}
