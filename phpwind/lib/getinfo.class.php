...
class GetInfo{
	...
	function userSort($type,$num){
		...
		if (in_array($type,array('postnum','onlinetime','rvrc','money','credit','currency','digests'))) {
			...
			if ($this->reality == false) {
				$sql = "SELECT f.uid AS id, COUNT(*) AS value, m.username AS addition FROM pw_friends f LEFT JOIN pw_members m USING(uid) GROUP BY uid";
			} else {
				$sql = "SELECT f.uid, COUNT(*) AS value, m.username, m.icon, m.groupid, m.memberid FROM pw_friends f LEFT JOIN pw_members m USING(uid) GROUP BY uid";
			}
