...
//	$rightset = P_unserialize($rightset);
	...
	InitGP(array('tid','thelast','authorid','pcid'),2);
	...
	InitGP(array('pcid','pcmid','tid','jointype','ifpay','payway'),2);
	...
	$rt = $db->get_one("SELECT nums,phone,mobile,address,extra,name,zip,message FROM pw_pcmember WHERE pcmid=".pwEscape($pcmid)." AND uid=".pwEscape($winduid));
	if (empty($_POST['step'])) {
...
function addSingleFriend($updatenum,$winduid,$frienduid,$timestamp,$status,$friendtype=0,$checkmsg=''){
	global $db;
	$friend = $db->get_one("SELECT * FROM pw_friends WHERE uid=".$winduid." AND friendid=".$frienduid);
	if(!$friend){
		$pwSQL = pwSqlSingle(array(
			'uid'		=> $winduid,
			'friendid'	=> $frienduid,
	...
	($upmember) && $db->update("UPDATE pw_memberdata SET f_num=f_num+1 WHERE uid=".pwEscape($winduid));
}
?>