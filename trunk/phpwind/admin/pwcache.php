...
if (empty($action)) {
	...
	if (!$type) {
		...
		if (in_array($type,array('replysort','replysortday','replysortweek'))){
			...
			for ($j=0;$j<5;$j++) {
				$replysort_judge = '';
				@include_once(D_P.'data/bbscache/'.Pcv("replysort_judge_".$j).'.php');
			...
			for ($i=0;$i<5;$i++) {
				if ($step < $total) {
					...
					if ($forum[$fourmid]['type']=='category') {
						...
						if ($arr_posts) {
							$sql = "INSERT INTO pw_elements(id,value,addition,special,type,mark) VALUES".pwSqlMulti($arr_posts,false);
							$db->update($sql);
						}
					}
				} else {
			...
			for ($i=0;$i<5;$i++) {
				if ($step < $total) {
					...
					if ($forum[$fourmid]['type']=='category') {
						...
						if ($arr_posts) {
							$sql = "INSERT INTO pw_elements(id,value,type,mark) VALUES".pwSqlMulti($arr_posts,false);
							$db->update($sql);
						}
					}
				} else {
