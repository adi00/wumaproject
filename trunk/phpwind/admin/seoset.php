...
if ($action == 'update') {
	...
	updatecache_f();
	$db_bbstitle = array('index'=>Char_cv(strip_tags($contents['title_index'])),
						 'thread'=>Char_cv(strip_tags($contents['title_thread'])),
						 'read'=>Char_cv(strip_tags($contents['title_read'])));
	$db_metadescrip = array('index'=>Char_cv(strip_tags($contents['metadesc_index'])),
					        'thread'=>Char_cv(strip_tags($contents['metadesc_thread'])),
					        'read'=>Char_cv(strip_tags($contents['metadesc_read'])));
	$db_metakeyword = array('index'=>Char_cv(strip_tags($contents['metakeyword_index'])),
		...
		foreach ($categorys as $k1 => $category) {
			foreach ($forums as $k2 => $forum) {
				if ($forum['fup'] == $category['fid']) {
					$forum['limage1'] = '<span class="fourm-two"></span>';
					$forum['limage'] = '<span class="fourm-one"></span>';
					$forumList[$category['fid']][] = $forum;
					foreach ($subForums1 as $k3 => $subf1) {
						if ($subf1['fup'] == $forum['fid']) {
							$subf1['limage1'] = '<span class="fourm-two"></span><span class="fourm-two"></span>';
							$subf1['limage'] = '<span class="fourm-two"></span><span class="fourm-one"></span>';
							$forumList[$category['fid']][] = $subf1;
							foreach ($subForums2 as $k4 => $subf2) {
								if ($subf2['fup'] == $subf1['fid']) { 
									$subf2['limage1'] = '<span class="fourm-two"></span><span class="fourm-two"></span><span class="fourm-two"></span>';
									$subf2['limage'] = '<span class="fourm-two"></span><span class="fourm-two"></span><span class="fourm-one"></span>';
