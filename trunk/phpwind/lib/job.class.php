...
class PW_Job {
	...
	function jobApplyController($userId,$jobId){
		...
		if($jober && $jober['total']>0 && $job['period']>0 && $jober['status']>1 ){
	...
	function getJobLists($checked){
		...
		$jobHtml = $jobInfo = "";
		foreach($jobs as $k=>$v){
			$jobHtml .= '<li id="'.$k.'"><a href="javascript:;" hidefocus="true"><em></em>'.$v.'</a></li>';
	...
	function getJobData($id,$data,$checked){
         $html = '<ul id="job_'.$id.'" style="display:none;">';
         foreach($data as $k=>$v){
         	$checkHtml = ($checked == $k) ? "checked" : "";
         	$html .= '<li><a href="javascript:;" hidefocus="true"><input name="factor[job]" type="radio" value="'.$k.'" '.$checkHtml.'/> '.$v.'</a></li>';
         }
         $html .= '</ul>';
         return $html;
	}
	...
	function getLevelCheckbox($checkeds=array(),$name='usergroup[]',$id='usergroup'){
		...
		$html = '';
		$html .= "<dl class=\"task-mng-group cc\"><dt>系统组：</dt><dd>".$this->_buildCheckbox($result['system'],$name,$id,$checkeds)."</dd></dl><div class=\"c\"></div>";
		$html .= "<dl class=\"task-mng-group cc\"><dt>会员组：</dt><dd>".$this->_buildCheckbox($result['member'],$name,$id,$checkeds)."</dd></dl><div class=\"c\"></div>";
		$html .= "<dl class=\"task-mng-group cc\"><dt>特殊组：</dt><dd>".$this->_buildCheckbox($result['special'],$name,$id,$checkeds)."</dd></dl><div class=\"c\"></div>";
	...
	function _buildCheckbox($arrays,$name,$id,$checkeds=array()){
		...
		foreach($arrays as $k=>$v){
			...
			$html .= '<span><input type="checkbox" value="'.$k.'" name="'.$name.'" id="'.$id.'" '.$checked.'/> '.$v."</span>";
		}
		return $html;
	}
