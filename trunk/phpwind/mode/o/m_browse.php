...
function browseWrite($num) {
	...
	$query = $db->query("SELECT wd.*,m.username,m.icon,m.groupid FROM pw_owritedata wd LEFT JOIN pw_members m ON wd.uid=m.uid WHERE wd.touid='' $sqladd ORDER BY wd.id DESC LIMIT $num");
	while ($rt = $db->fetch_array($query)) {
