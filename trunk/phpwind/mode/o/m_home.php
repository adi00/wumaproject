...
if (!empty($frienddb)) {
	if (!empty($feed_frienddb)){
		...
		$query = $db->query('SELECT f.*,m.username,m.groupid,icon FROM pw_feed f LEFT JOIN pw_members m ON f.uid=m.uid WHERE f.uid IN('.pwImplode($frienddb).") $addwhere ORDER BY timestamp DESC LIMIT 30");
		while ($rt = $db->fetch_array($query)) {
