...
/**** 帖子排序  板块-帖子*****/
if ($ordertype) {
	if (GetCookie('rorder') && count(GetCookie('rorder')) > 10) {
		Cookie("rorder",'',0);
	}
...
/*	$metadescrip = stripWindCode($read['content']);
	...
	if (trim($metadescrip)) {
		$metadescrip = $metadescrip.' '.$db_bbsname;
		$pw_seoset->set_default($metadescrip);
	}
...
if ($groupid=='guest' && !$read['ifshield']/* && !isban($read,$fid)*/) {
	...
	$pw_seoset->set_types($topic_type_cache[$fid],$read['type']);
}
...
//帖子详细内容
$ping_logs = array();
$pageinverse && $start_limit += $db_readperpage - 1;
...
function getThreadType(){
	global $topic_type_cache,$read,$fid;
	$topic_type = isset($topic_type_cache[$fid][$read['type']]) ? $topic_type_cache[$fid][$read['type']] : '';
